...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        # raiseNotDefined()
        cur_factors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for eliminationVar in eliminationOrder:  # 根据要求按照顺序遍历需要消除的变量
            cur_factors, joinedFactor = joinFactorsByVariable(cur_factors, eliminationVar)
            # 当非条件变量个数不为1时，消除变量
            if len(joinedFactor.unconditionedVariables()) > 1:
                joinedFactor = eliminate(joinedFactor, eliminationVar)
                cur_factors.append(joinedFactor)
            else:  # 否则将其丢弃
                continue
        joinedFactor = joinFactors(cur_factors)
        "*** END YOUR CODE HERE ***"
